import { GrpcEvent } from "@/src/gen/event_pb";
import { GrpcMessageEvent } from "@/src/gen/message_pb";
...
class Socket extends EventEmitter {
  ...
  onMessage(event) {
    if (event.data instanceof ArrayBuffer) {
      // var dv = new DataView(event.data, 0);
      // console.log(dv.getInt32()); // get type
      
      // // parse data
      // console.log(GrpcMessageEvent.fromBinary(new Uint8Array(event.data, 4)))
      console.log(GrpcEvent.fromBinary(new Uint8Array(event.data)));
      
      const { event: eventData } = GrpcEvent.fromBinary(new Uint8Array(event.data));
      const { case: eventCase, value } = eventData;
      const { data, type } = value;

      const eventListener = `${eventCase}:${type}`;

      console.log(eventListener, data);
      super.emit(eventListener, data);
